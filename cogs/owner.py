...
log = logging.getLogger("moobot.owner")
...
class Owner:
    ...
    def __init__(self, bot):
        ...
        self.disabled_commands = dataIO.load_json("data/moobot/disabled_commands.json")
        self.global_ignores = dataIO.load_json("data/moobot/global_ignores.json")
    ...
    async def _set(self, ctx):
        """Changes MooBot's core settings"""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start MooBot "
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets MooBot's prefixes for this server
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets MooBot's streaming status
    ...
    async def _blacklist_add(self, user: GlobalUser):
        """Adds user to MooBot's global blacklist"""
    ...
    async def _whitelist_add(self, user: GlobalUser):
        """Adds user to MooBot's global whitelist"""
    ...
    async def _whitelist_remove(self, user: GlobalUser):
        """Removes user from MooBot's global whitelist"""
    ...
    async def shutdown(self, silently : bool=False):
        ...
        try: # We don't want missing perms to stop our shutdown
            if not silently:
                await self.bot.say("Shutting down... " + wave)
        except:
            pass
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart MooBot

        Makes MooBot quit with exit code 26
    ...
    @commands.command()
    async def info(self):
        """Shows info about MooBot"""
        author_repo = "https://github.com/evanq123"
        moobot_repo = author_repo + "/MooBot"
        server_url = "https://discord.gg/MooBot"
        dpy_repo = "https://github.com/Rapptz/discord.py"
        python_url = "https://www.python.org/"
        since = datetime.datetime(2017, 12, 23, 0, 0)
        ...
        about = (
            "This is an instance of [MooBot, a private Discord bot]({}) "
            "created by [evanq123]({}).\n\n"
        ...
        embed.add_field(name="About MooBot", value=about, inline=False)
        embed.set_footer(text="Bringing joy since 23 Dec 2017 (over "
    ...
    async def version(self):
        """Shows MooBot's current version"""
...
def check_files():
    if not os.path.isfile("data/moobot/disabled_commands.json"):
        print("Creating empty disabled_commands.json...")
        dataIO.save_json("data/moobot/disabled_commands.json", [])

    if not os.path.isfile("data/moobot/global_ignores.json"):
        ...
        dataIO.save_json("data/moobot/global_ignores.json", data)
