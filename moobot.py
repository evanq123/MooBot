...
#
# MooCow, a Discord bot by MooCow, based on discord.py and Red's modularization.
#

description = "MooCow - A local Discord bot by MooCow"
...
class Bot(commands.Bot):
        ...
        if self._message_modifiers:
            ...
            for m in self._message_modifiers:
                try:
                    ...
                    # Faulty modifiers should not break send_message
                    pass
    ...
    async def shutdown(self, *, restart=False):
        """Quits MooBot with exit code 0

        If restart is True, exit code will be 26 instead
        The launcher automatically restarts MooCow when that happens"""
        self._shutdown_mode = not restart
        await self.logout()

    def add_message_modifier(self, func):
        """
        Adds a message modifer to the bot
        ...
        value.
        Exceptions thrown will be silenced.
        """
    ...
    async def pip_install(self, name, *, timeout=None):
        ...
        if IS_MAC:
            args.remove("--target") # --target bugs on Homebrew.
            args.remove("lib")
...
def initialize(bot_class=Bot, formatter_class=Formatter):
        ...
        if bot.settings.owner:
            ...
            if not owner:
                try:
                    ...
                    owner = None
                if not owner:
                    owner = bot.settings.owner # Just ID
            return owner
    ...
    async def on_ready():
        ...
        if bot.settings.token and not bot.settings.self_bot:
            print("\nUse this url to bring  MooBot to a server:")
...
def check_folders():
    folders = ("data", "data/moobot", "cogs", "cogs/utils")
    ...
    if not settings.login_credentials:
        ...
        if first_run:
            ...
            if settings.default_admin == "":
                settings.default_admin = "Administrators"
            settings.save_settings()

            print("\nInput the moderator role's name. Anyone with this role in "
                  "Discord will be able to use the bot's mod commands")
            print("Leave blank for default name (Moderators)")
            settings.default_mod = input("\nModerator role> ")
            if settings.default_mod == "":
                settings.default_mod = "Moderators"
            settings.save_settings()

            print("\nThe configuration is done. Leave this window always open "
                  "to keep MooBot online.\nAll commands will have to be issued "
                  "through Discord's chat, *this window will now be read "
                  "only*.\nPlease read this guide for a good overview on how "
...
def set_logger(bot):
    logger = logging.getLogger("moobot")
    logger.setLevel(logging.INFO)

    moobot_format = logging.Formatter(
        '%(asctime)s %(levelname)s %(module)s %(funcName)s %(lineno)d: '
        '%(message)s',
        datefmt="[%d/%m/%Y %H:%M]")

    stdout_handler = logging.StreamHandler(sys.stdout)
    stdout_handler.setFormatter(moobot_format)
    ...
    fhandler = logging.handlers.RotatingFileHandler(
        filename='data/moobot/moobot.log', encoding='utf-8', mode='a',
        maxBytes=10**7, backupCount=5)
    fhandler.setFormatter(moobot_format)
    ...
    handler = logging.FileHandler(
        filename='data/moobot/discord.log', encoding='utf-8', mode='a')
...
def set_cog(cog, value):  # TODO: R elocate this function
    data = dataIO.load_json("data/moobot/cogs.json")
    data[cog] = value
    dataIO.save_json("data/moobot/cogs.json", data)


def load_cogs(bot):
    defaults = ("audio", "mod")  # TODO: Add more default cogs

    try:
        registry = dataIO.load_json("data/moobot/cogs.json")
    except:
        registry = {}
    ...
    if owner_cog is None:
        print ("The 'owner' cog is missing. It is a core cog that is needed "
               "for MooBot to function. Please begin the reinstall process.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Skipping initial cogs loading (--no-cogs)")
        if not os.path.isfile("data/moobot/cogs.json"):
            dataIO.save_json("data/moobot/cogs.json", {})
        return

    failed = []
    extensions = owner_cog._list_cogs()

    if not registry: # All defefault cogs enabled by default
        for ext in defaults:
...
if __name__ == '__main__':
    ...
    try:
        ...
        bot.logger.exception("Fatal exception, attempting logout",
                             exc_info=e)
        loop.run_until_complete(bot.logout())
    finally:
        loop.close()
